...
public class PolylineEncoder {
    ...
    if (track.getTrackpoints().size() > 2) {
      ...
      while (stack.size() > 0) {
        ...
        for (i = current[0] + 1; i < current[1]; i++) {
          temp = this.distance(track.getTrackpoints().get(i), track.getTrackpoints()
              .get(current[0]), track.getTrackpoints().get(current[1]));
          if (temp > maxDist) {
            maxDist = temp;
            maxLoc = i;
  ...
  public double distance(Trackpoint p0, Trackpoint p1, Trackpoint p2) {
    ...
    if (p1.getLatDouble() == p2.getLatDouble() && p1.getLonDouble() == p2.getLonDouble()) {
      out = Math.sqrt(Math.pow(p2.getLatDouble() - p0.getLatDouble(), 2)
          + Math.pow(p2.getLonDouble() - p0.getLonDouble(), 2));
    } else {
      u = ((p0.getLatDouble() - p1.getLatDouble()) * (p2.getLatDouble() - p1.getLatDouble()) + (p0.getLonDouble() - p1.getLonDouble())
          * (p2.getLonDouble() - p1.getLonDouble()))
          / (Math.pow(p2.getLatDouble() - p1.getLatDouble(), 2) + Math.pow(
              p2.getLonDouble() - p1.getLonDouble(), 2));

      if (u <= 0) {
        out = Math.sqrt(Math.pow(p0.getLatDouble() - p1.getLatDouble(), 2)
            + Math.pow(p0.getLonDouble() - p1.getLonDouble(), 2));
      }
      if (u >= 1) {
        out = Math.sqrt(Math.pow(p0.getLatDouble() - p2.getLatDouble(), 2)
            + Math.pow(p0.getLonDouble() - p2.getLonDouble(), 2));
      }
      if (0 < u && u < 1) {
        out = Math.sqrt(Math.pow(p0.getLatDouble() - p1.getLatDouble() - u
            * (p2.getLatDouble() - p1.getLatDouble()), 2)
            + Math.pow(
                p0.getLonDouble() - p1.getLonDouble() - u * (p2.getLonDouble() - p1.getLonDouble()),
                2));
      }
    }
    return out;
  ...
  /**
   * @param points set the points that should be encoded all points have to be
   * in the following form: Latitude, longitude\n
   */
  public static Track pointsToTrack(String points) {
    ...
    while (st.hasMoreTokens()) {
      ...
      trk.addTrackpoint(new Trackpoint(new Double(pointStrings[0]), new Double(pointStrings[1])));
    }
    return trk;
  }

  /**
   * @param LineString set the points that should be encoded all points have to
   * be in the following form: Longitude,Latitude,Altitude"_"...
   */
  public static Track kmlLineStringToTrack(String points) {
    ...
    while (st.hasMoreTokens()) {
      String[] pointStrings = st.nextToken().split(",");
      trk.addTrackpoint(new Trackpoint(new Double(pointStrings[1]), new Double(pointStrings[0]),
          new Double(pointStrings[2])));
    }
    return trk;
  }
   ...
   */
  private String encodeLevels(ArrayList<Trackpoint> points, double[] dists, double absMaxDist) {
    int i;
